...
export const TaskColumn: React.FC<TaskColumnProps> = ({ title, status, tasks, onTransition, onSelectTask }) => {
    return (
        <div className="w-80 min-w-80 bg-gray-100 rounded-lg p-3 flex flex-col gap-3" data-status={status}>
            ...
            <div className="flex-1 min-h-[150px] flex flex-col gap-3 overflow-y-auto pr-1">
                {tasks.length === 0 ? (
                    ...
                    tasks.map(task => <TaskCard key={task.id} task={task} onTransition={onTransition} onSelectTask={onSelectTask} />)
                )}
            </div>
        </div>
    );
